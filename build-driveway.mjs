...
export function drivewayBody({ brand, cities, phoneIcon, footer, wizardScript }) {
  ...
  const photo = (name, alt, eager = false) => `<img src="/assets/projects/${name}-1200.webp" srcset="/assets/projects/${name}-480.webp 480w, /assets/projects/${name}-800.webp 800w, /assets/projects/${name}-1200.webp 1200w, /assets/projects/${name}-1600.webp 1600w" sizes="(max-width: 800px) 100vw, 55vw" width="1200" height="${name.includes('after-wide') ? 800 : 1800}" alt="${alt}" ${eager ? 'fetchpriority="high"' : 'loading="lazy"'} decoding="async">`;
  const questions = [
...
<main class="drive" id="main" tabindex="-1">
  ...
  <figure class="drive-result" id="project">
    ${photo('walnut-creek-exterior__after-wide', 'Completed Walnut Creek exterior project with concrete driveway, retaining wall and front porch')}
    <figcaption><span>Actual AZ Elevated Builders project</span><span>Walnut Creek · Concrete &amp; exterior work</span></figcaption>
  </figure>
  </section>
  ...
  <section class="drive-proof drive-section" aria-labelledby="proof-title">
    <div class="drive-proof__image">${photo('walnut-creek-exterior__finishing-slab', 'Concrete being hand-finished at the Walnut Creek project')}<p>Concrete finishing · Walnut Creek project</p></div>
